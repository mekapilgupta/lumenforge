<script lang="ts">
  ...
  const SERIES_TO_CATEGORY: Record<string, string> = {
    'california': 'WEDGES',
    'new jersey': 'WEDGES',
    'virginia': 'WEDGES',
    ...
    'miami': 'DAILY COMFORT',
    'miami 1': 'DAILY COMFORT',
    'miami 2': 'DAILY COMFORT',
    'miami 3': 'DAILY COMFORT',
    'bermuda': 'DAILY COMFORT'
  };

  const seriesTabs = ['WEDGES', 'FLATS', 'DAILY COMFORT'];
  let activeTab = $state('WEDGES');
  ...
  const catalogList = $derived.by(() => {
    console.log('[BestSellers] Deriving catalogList. dbProducts length:', dbProducts.length);
    if (dbProducts.length > 0) {
      const grouped = groupProductsFromDB(dbProducts);
      console.log('[BestSellers] Derived grouped products from DB. Length:', grouped.length);
      return grouped;
    } else {
      console.log('[BestSellers] dbProducts is empty. Returning empty list.');
      return [];
    }
  });

  // Filter color variant cards matching the selected series tab mapped category
  const activeSeriesCards = $derived.by(() => {
    const filtered = catalogList.filter(p => {
      const seriesKey = (p.series || '').toLowerCase();
      const cat = SERIES_TO_CATEGORY[seriesKey] || 'FLATS';
      return cat === activeTab;
    });
    console.log(`[BestSellers] Filtered activeSeriesCards for tab "${activeTab}". Matched count:`, filtered.length);
    return filtered;
  });
  ...
  function groupProductsFromDB(prods: any[]) {
    console.log('[BestSellers] Grouping DB products. Raw prods count:', prods.length);
    return prods.map(p => {
      try {
        ...
        const mappedImage = imagesList[0] || p.thumbnail_url || '/placeholder.jpg';
        console.log(`[BestSellers] Product ID "${p.id}" (${p.name}): Image path resolved to: "${mappedImage}"`);
        ...
        console.error(`[BestSellers] Error mapping product ID "${p?.id}":`, err);
        return null;
      }
    }).filter(Boolean);
  }
  ...
  // Open Lightbox Modal for variant card
  function openLightbox(card: any) {
    // If we have other variants in the active series, collect all their images to swipe through
    lightboxImages = activeSeriesCards.map(c => c.image);
    lightboxActiveIdx = activeSeriesCards.findIndex(c => c.id === card.id);
    if (lightboxActiveIdx === -1) lightboxActiveIdx = 0;
    isLightboxOpen = true;
  }

  // Custom visual background style generator for cards
...
<section class="py-16 bg-[#FDFBF7] border-b border-[#f0e0e8]/30">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <!-- Tabs row -->
    <div class="flex justify-center flex-wrap gap-2 md:gap-3 mb-10">
      {#each seriesTabs as tab}
        <button
          onclick={() => activeTab = tab}
          class="px-6 py-3 rounded-full font-bold uppercase tracking-wider text-xs md:text-sm border transition-all duration-300 shadow-sm cursor-pointer focus:outline-none"
          style="
            background: {activeTab === tab ? 'var(--color-brand-magenta)' : 'white'};
            color: {activeTab === tab ? 'white' : 'var(--color-text-mid)'};
            border-color: {activeTab === tab ? 'var(--color-brand-magenta)' : 'rgba(216, 27, 96, 0.15)'};
            transform: scale({activeTab === tab ? 1.05 : 1});
          "
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {#each activeSeriesCards as card (card.id)}
        ...
        >
          ...
          <!-- Card text info (20% visual height) -->
